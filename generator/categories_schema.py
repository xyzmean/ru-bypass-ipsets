...
CATEGORIES = [
    ...
    },
    {
        ...
        "default_on": False,
        "is_geoblock": False,
        # Вычитания по этому списку не гоняются, и это измерено на выпущенном файле, а не
        # предположено. Без no_subtract от списка остаётся меньше трети:
        # 151.101.130.0/24 (downloads.openwrt.org) вычитается как край Fastly,
        # 64.226.122.0/24 (openwrt.org) и 167.235.28.0/24 (git) — как диапазоны
        # DigitalOcean и Hetzner. Первый из них — ровно тот, ради которого список и нужен:
        # с него роутер берёт пакеты. Список из второстепенных узлов хуже, чем отсутствие
        # списка, потому что выглядит рабочим.
        #
        # Плата названа вслух, и в адресах: на выпущенном openwrt.lst (14 сетей, 4608
        # адресов) вычитание оставило бы 4 сети и 2048 адресов, то есть включивший этот
        # список уводит в туннель 2560 адресов — 1280 у Fastly, за которыми живут чужие
        # сайты, и 1280 у Hetzner с DigitalOcean. Это 56% списка, и это осознанный выбор
        # человека, а не умолчание: категория выключена по умолчанию.
        #
        # Числа здесь не сверяются глазами: ту же цену печатает сборка (no_subtract_cost),
        # и её же пересчитывает по выпущенному файлу стенд tests/gate_shared_proxy.py.
        "no_subtract": True,
