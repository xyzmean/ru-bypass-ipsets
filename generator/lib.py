...
def punch_out_counted(nets, holes):
    """То же, что punch_out, плюс сколько ВХОДНЫХ сетей дырки задели.

    Счётчик отдаётся отсюда, а не считается вызывающим, и это не удобство подписи. Оба
    вызывающих (вычитание общих прокси и вычитание инфраструктуры) считали его так:

        touched = sum(1 for n in nets if any(n.overlaps(r) for r in ranges))

    то есть честным перебором всех дырок на каждую сеть — рядом с функцией, которая
    существует ровно затем, чтобы этого перебора не делать. Замер на выпущенном rkn.lst
    (11 221 префикс) против 1 323 схлопнутых диапазонов прокси: перебор 10,53 с, а само
    вычитание — 0,04 с. Счётчик «сколько задето» был в двести пятьдесят раз дороже работы,
    которую он описывает; на входных объёмах категории rkn (около 40 тыс. префиксов до
    finalize) это порядка сорока секунд на одну категорию.

    Здесь он достаётся бесплатно: обход по дыркам уже сделан, надо только заметить, вошёл
    ли в него хоть один шаг.
    """
    if not holes:
        return list(nets), 0
...
def punch_out(nets, holes):
    ...
    """
    return punch_out_counted(nets, holes)[0]
